...
bool vemath::saveVectorPoint2DToFile(const std::vector<Point2D> &data, const string &fileName, unsigned long long N) {
    ...
    N = N==0 ? data.size() : N;
    for(int i = 0; i < N; i++) {
        _ofstream << data[i].x << "   " << data[i].y << endl;
    }
...
[[nodiscard]] Point3D randomDirection(int seed) {
    ...
    Point3D dir = {1, 1, 1};
    while(dir.abs() > 1) {
        dir.setx(-1 + 2*rand()/RAND_MAX);
        dir.sety(-1 + 2*rand()/RAND_MAX);
        dir.setz(-1 + 2*rand()/RAND_MAX);
    }
    dir = dir.normalize();
    return dir;
}
